...
class BaseTransform(BaseModule):
    ...
    def get_geometry_v1(
        ...
        **kwargs,
        ...
        iH = img_metas[0]["img_shape"][0][0] # 480
        iW = img_metas[0]["img_shape"][0][1] # 800
    ...
    def forward(self, images, img_metas):
        ...
        for img_meta in img_metas: # here process with batch mode
...
class LSSTransform(BaseTransform):
    ...
    def forward(self, images, img_metas):
        ...
        x = self.downsample(x) # x stands for img-bev already
        ret_dict = dict(bev=x, depth=depth)
        return ret_dict
    
    def forward_trt(self, images, img_metas):
        x, depth = super().forward(images, img_metas)
        x = self.downsample(x)
        return x, depth
